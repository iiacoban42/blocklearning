...
class Contract():
    ...
    def get_submissions_for_scoring(self):
        [round, trainers, submissions] = self.contract.functions.getUpdatesForScore(
        ).call(self.default_opts)
        return (round, trainers, submissions)

    def get_submissions_for_aggregation(self):
        [round, trainers, submissions] = self.contract.functions.getUpdatesForAggregation(
        ).call(self.default_opts)
        return (round, trainers, submissions)
    ...
    def submit_submission(self, submission):
        ...
        tx = self.contract.functions.submitUpdate(
            submission).transact(self.default_opts)
        return tx, self.__wait_tx(tx)

    def submit_scorings(self, trainers, scores):
        self.__unlock_account()
        tx = self.contract.functions.submitScores(
            trainers, scores).transact(self.default_opts)
        return tx, self.__wait_tx(tx)

    def submit_aggregation(self, weights_id):
        self.__unlock_account()
        tx = self.contract.functions.submitAggregation(
            weights_id).transact(self.default_opts)
        return tx, self.__wait_tx(tx)
    ...
    def submit_aggregation_with_gradients(self, weights_id, trainers, gradients_ids):
        ...
        tx = self.contract.functions.submitAggregationWithGradients(
            weights_id, trainers, gradients_ids).transact(self.default_opts)
        return tx, self.__wait_tx(tx)
    ...
    def start_round(self, *args):
        self.__unlock_account()
        tx = self.contract.functions.startRound(
            *args).transact(self.default_opts)
        return tx, self.__wait_tx(tx)
    ...
    def __wait_tx(self, tx):
        self.log.info(json.dumps(
            {'event': 'tx_start', 'tx': tx.hex(), 'ts': time.time_ns()}))
        receipt = self.web3.eth.wait_for_transaction_receipt(tx)
        self.log.info(json.dumps({'event': 'tx_end', 'tx': tx.hex(
        ), 'gas': receipt.gasUsed, 'ts': time.time_ns()}))
        return receipt

    def __unlock_account(self):
        self.web3.geth.personal.unlock_account(
            self.account, self.passphrase, 600)
