...
def main(provider, abi, contract, scoring, trainers,  data_dir, val, rounds):
    ...
    def choose_participants():
        ...
        if trainers == 'random':
            ...
            round_trainers = random.randint(
                len(all_trainers) // 2, len(all_trainers))
        elif trainers == 'all':
    ...
    def eval_model(weights):
        log.info(json.dumps({'event': 'eval_start',
                 'ts': time.time_ns(), 'round': round}))
        model.set_weights(weights_loader.load(weights))
        metrics = model.evaluate(x_val, y_val)
        accuracy = metrics['sparse_categorical_accuracy']
        log.info(json.dumps(
            {'event': 'eval_end', 'ts': time.time_ns(), 'round': round}))
        return accuracy

    for i in range(0, rounds):
        round_trainers, round_aggregators, round_scorers = choose_participants()
        log.info(json.dumps({'event': 'start', 'trainers': round_trainers,
                 'aggregators': round_aggregators, 'scorers': round_scorers, 'ts': time.time_ns()}))

        if trainers == 'fcfs':
            contract.start_round(round_trainers, round_aggregators)
        elif scoring != 'none':
            contract.start_round(
                round_trainers, round_aggregators, round_scorers)
        else:
            contract.start_round(round_trainers, round_aggregators)
        ...
        log.info(json.dumps({'event': 'end', 'ts': time.time_ns(
        ), 'round': round, 'weights': weights, 'accuracy': butilities.float_to_int(accuracy)}))
