    ...
    def aggregate(self):
        ...
        if self.logger is not None:
            self.logger.info(json.dumps(
                {'event': 'start', 'submissions': submissions, 'round': round, 'ts': time.time_ns()}))

        if self.logger is not None:
            self.logger.info(json.dumps(
                {'event': 'fedavg_start', 'round': round, 'ts': time.time_ns()}))

        outputs = [self.datastore.load(output_id)
                   for (_, _, _, output_id) in submissions]
        ...
        server_grads = tape.gradient(
            loss_value, self.model.model.trainable_weights)
        ...
        metrics = {m.name: m.result().numpy()
                   for m in self.model.model.metrics}
        ...
        client_grads_ids = [self.datastore.store(
            grad) for grad in client_grads]

        self.contract.submit_aggregation_with_gradients(
            weights_id, trainers, client_grads_ids)

        if self.logger is not None:
            self.logger.info(json.dumps({'event': 'end', 'round': round, 'submissions': submissions, 'ts': time.time_ns(
            ), 'new_weights': weights_id, 'client_grads': client_grads_ids, 'accuracy': float_to_int(accuracy)}))

    # Private utilities
    def __register(self):
        if self.logger is not None:
            self.logger.info(json.dumps(
                {'event': 'checking_registration', 'ts': time.time_ns()}))

        self.contract.register_as_aggregator()

        if self.logger is not None:
            self.logger.info(json.dumps(
                {'event': 'registration_checked', 'ts': time.time_ns()}))
