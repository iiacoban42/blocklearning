...
def main(provider, ipfs, abi, account, passphrase, contract, log, val, scoring):
    ...
    contract = blocklearning.Contract(
        log, provider, abi, account, passphrase, contract)
    ...
    if scoring == 'marginal-gain':
        aggregator = aggregators.FedAvgMeansAggregator(
            model.count, weights_loader)
    elif scoring == 'multi-krum':
        aggregator = aggregators.MultiKrumAggregator(
            model.count, weights_loader)
    elif scoring == 'blockflow':
        aggregator = aggregators.BlockFlowAggregator(
            model.count, weights_loader)
    else:
        aggregator = aggregators.FedAvgAggregator(model.count, weights_loader)

    aggregator = blocklearning.Aggregator(
        contract, weights_loader, model, aggregator, with_scores=scoring != 'none', logger=log)
