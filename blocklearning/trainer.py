    ...
    def train(self):
        ...
        if self.logger is not None:
            self.logger.info(json.dumps(
                {'event': 'start', 'round': round, 'weights': weights_id, 'ts': time.time_ns()}))
        ...
        if self.logger is not None:
            self.logger.info(json.dumps(
                {'event': 'train_end', 'round': round, 'ts': time.time_ns()}))

        trainingAccuracy = float_to_int(
            history['sparse_categorical_accuracy'][0])
        validationAccuracy = float_to_int(
        ...
        if self.logger is not None:
            self.logger.info(json.dumps(
                {'event': 'end', 'round': round, 'weights': weights_id, 'ts': time.time_ns(), 'submission': submission}))
