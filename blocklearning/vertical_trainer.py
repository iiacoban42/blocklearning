...
class VerticalTrainer():
    ...
    def forward(self):
        ...
        submission = {
            ...
            # We use the weights for the intermediate output in the vertical case.
            'weights': output_id
        }
        ...
        if self.logger is not None:
            self.logger.info(json.dumps({'event': 'forward_end', 'round': round,
                             'weights': output_id, 'ts': time.time_ns(), 'submission': submission}))
    ...
    def backward(self):
        ...
        if self.logger is not None:
            self.logger.info(json.dumps(
                {'event': 'backward_end', 'round': round, 'output_grads_id': output_grads_id, 'ts': time.time_ns()}))
